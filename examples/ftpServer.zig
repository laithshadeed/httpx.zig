...
pub fn main() !void {
    ...
    const ClientThread = struct {
        fn run(serverPort: u16, threadAllocator: std.mem.Allocator) void {
            var client = httpx.ftp.Client.connect(threadAllocator, .{
                .host = "127.0.0.1",
                .port = serverPort,
                .user = "demo",
                .password = "password",
    ...
    const t = try std.Thread.spawn(.{}, ClientThread.run, .{ port, allocator });
