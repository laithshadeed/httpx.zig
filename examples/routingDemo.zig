...
pub fn main() !void {
    ...
    const profileUrl = try server.router.url("api-user", .{ .id = 7 });
    defer allocator.free(profileUrl);
    try check(std.mem.eql(u8, profileUrl, "/api/users/7"), "reverse api-user");
    ...
    const url1 = try std.fmt.bufPrint(&urlBuf, "http://127.0.0.1:{d}/api/users/42", .{port});
    ...
    const url2 = try std.fmt.bufPrint(&urlBuf, "http://127.0.0.1:{d}/api/users/abc", .{port});
    const r2 = try get(&client, url2);
    defer allocator.free(r2.body);
    try check(r2.status == 404, "GET int rejects abc");

    const url3 = try std.fmt.bufPrint(&urlBuf, "http://127.0.0.1:{d}/api/users/3/posts/4", .{port});
    ...
    const url4 = try std.fmt.bufPrint(&urlBuf, "http://127.0.0.1:{d}/blog/hello-world", .{port});
    ...
    const url5 = try std.fmt.bufPrint(&urlBuf, "http://127.0.0.1:{d}/files/a/b/c.txt", .{port});
    ...
    const url6 = try std.fmt.bufPrint(&urlBuf, "http://127.0.0.1:{d}/link", .{port});
    var r6 = try client.post(url6, .{});
    defer r6.deinit();
    try check(r6.status == 405, "POST /link is 405");
    try check(r6.header("Allow") != null, "405 carries Allow");

    const url7 = try std.fmt.bufPrint(&urlBuf, "http://127.0.0.1:{d}/missing", .{port});
