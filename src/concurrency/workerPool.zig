...
pub const Pool = struct {
    ...
    var gNoop align(8) = std.atomic.Value(bool).init(false);
...
test "pool cancel queued tasks" {
    ...
    var cancelFlag = std.atomic.Value(bool).init(false);
    try p.submit(dummy, null, &cancelFlag);
    p.cancelQueued();
    try p.start();
    p.shutdownDrain();
}
